    ...
    for i in models:
        ...
        # 手动处理下deeph的case
            ...
            if not os.path.exists(temppath):
                os.mkdir(temppath)
            os.system("cp -r {} {}".format(folder_path+'/*',temppath+'/'))
            add_key_to_json(jsonpath=temppath+'/train_set.json', content={"e3nn_ver":e3nn_ver})
...
def infer_and_eval(source, target):
    ...
    for i in models:
        ...
        for twist_case in ['2-1','3-2']:
            ...
            content=[temp_workpath+'predict_e3nn',
            ...
            with open(twistpath_local+'/inference.ini','w',encoding='utf-8') as f:
                f.writelines(jsoninfo)
            #print(i)
            ...
            with open(twistpath_local+'/predict_e3nn/eval.ini','w',encoding='utf-8') as f:
                f.writelines(jsoninfo)
            ...
            get mae
            '''
            content=[temp_workpath+'predict_e3nn/hamiltonians_pred.h5',
            temp_workpath+'predict_e3nn',
            temp_workpath+'mae/'
            ]
            ...
            with open(twistpath_local+'/mae/mae_heat_map.py','w',encoding='utf-8') as f:
                f.writelines(jsoninfo)

            cmd.append('cd {} && python mae_heat_map.py>>info_mae && sleep 10 \n'.format(temp_workpath+'mae',e3eval_path))
    ...
    with open(infer_path+'runall.sh','w',encoding='utf-8') as f:
        f.writelines(cmd)
    print(" Infer and eval file prepared for {} twisted material(s)! ".format(len(to_infer_list)))
