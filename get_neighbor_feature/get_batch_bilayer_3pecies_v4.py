...
def find_basis(x, b):
    ...
    with open('/home/tingbao/Desktop/2D_twist_database/code/template/OPMX/opmx_basis.txt', 'r') as opmx_basis:
...
str_unit = Structure.from_file('POSCAR')  # read in unit-cell bilayer structure
...
shift_list = shift_2d_linear(args.num[0], args.num[1])
for shift_index in range(args.num[0] * args.num[1]):
    ...
    str_shift.make_supercell([[args.supercell[0], 0, 0], [0, args.supercell[1], 0], [0, 0, 1]])
    
    for pert_index in range(args.num[2]):
        super_cart_coords = str_shift.cart_coords

        pert_x = (np.random.rand(len(str_shift), 1) - 0.5) * (args.pert[0] * 2)
        pert_y = (np.random.rand(len(str_shift), 1) - 0.5) * (args.pert[1] * 2)
        pert_z = (np.random.rand(len(str_shift), 1) - 0.5) * (args.pert[2] * 2)
...
"""
        ...
        if args.soc == True:  # set calc whether SOC
