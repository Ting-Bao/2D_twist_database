...
def main():
    ...
    for i in namelist:
        ...
        if not os.path.exists(socfilepath + i + '_soc/openmx.Band.json'):
            os.system('cd {} && python {}'.format(socfilepath + i + '_soc',
                                                  '../../../' + plotfile))
        if not os.path.exists(socfilepath + i + '_nosoc/openmx.Band.json'):
            os.system('cd {} && python {}'.format(socfilepath + i + '_nosoc',
                                                  '../../../' + plotfile))
        if not os.path.exists(tofile + i + '/nosoc-soc.compare.json'):
            shutil.copy(socfilepath + i + '_soc/openmx.Band.json',
                        tofile + i + '/soc.Band.json')
            shutil.copy(socfilepath + i + '_nosoc/openmx.Band.json',
                        tofile + i + '/nosoc.Band.json')
            os.system(
                'cd {}&& python {} -u 6.0 -d -6.0 --n1 {} --n2 {}'.format(
                    tofile + i, '../../../../' + comparefile, 'soc.Band.json',
                    'nosoc.Band.json'))

    for i in namelist:
        if rule1(socfilepath + i + '_soc/POSCAR') is False:
            socresult[i] = False
        else:
            socresult[i] = rule2(tofile + i + '/nosoc-soc.compare.json')

    pd.DataFrame.from_dict(socresult,
                           orient='index').to_excel(tofile + 'data.xlsx')
    pd.DataFrame.from_dict(
        socresult, orient='index').to_excel('data/finalchoice/' + 'ifsoc.xlsx')
