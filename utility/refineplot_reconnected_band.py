#!/usr/bin/env python
# Author: BaoTing@cmt.tsinghua for RcBand, plot_broken_y and compare band
...
def compare_plot(band_data_obj,savepath='./',auto_align = True, manual_shift=None, highlight_band=[]):
  '''band plot function
...
def load_process_band(readpath,savepath,refine_fermi=True,get_bandwidth=True,\
  advise_plot_range=True,plot=True,compare_dft=True,dft_path=None,usescatter=None):
  '''
  by baot:
  ...
  if compare_dft:
    ...
    shift = compare_plot(band_data_obj,savepath=savepath,highlight_band=highlight)
