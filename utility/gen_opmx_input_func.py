...
pot_path = '/home/xurz/POT/DFT_DATA19' # should be available in the calc env
pot_config='/home/tingbao/Desktop/2D_twist_database/code/template/OPMX/opmx_basis.txt' #should be available locally
...
def find_basis(x, b):
    ...
    with open(pot_config, 'r') as opmx_basis:
...
def gen_openmx_input(frompath='POSCAR',topath='./',\
    basis=1,bands=True,magmom=[0.,0.,0.],soc=True,dftu=False,dftu_value=['Mo','1d','3.0']):

    str_raw = Structure.from_file(frompath)  # read in structure
    ...
    structure.to('poscar', frompath+'_crystal')  # cp POSCAR to POSCAR_crystal
    ...
    for j in structure.formula.split():  # for element
        ...
        atomic_num, pot_name, n_elect, basis_set = find_basis(element_j, basis)
        ...
        for m in ion_orb_name:  # for orbital s,p,d,f
            for n in range(ion_orb_num[ion_orb_name.index(m)]):  # for orb_occup num
                if element_j == dftu_value[0] and str(n+1)+m == dftu_value[1]:
                    dftu_str += str(n+1) + m + ' ' + dftu_value[2] + ' '  # write dftu setting for each ion and orbital
                else:
    ...
    for i in range(len(str_raw)):
        ...
        coords_set_dftu = dftu * 'off'  # orbital polarization
...
"""
    ...
    if round(angle_degree) == 60:  # judge lattice angle
        band_key = bands * band_path_60
    elif round(angle_degree) == 120:
        band_key = bands * band_path_120
    elif round(angle_degree) == 90:
        band_key = bands * band_path_90
    else:
        band_key = ''
...
### END ###
"""
    with open(topath+'openmx_in.dat', 'w') as f:
        f.write(in_file)
