...
parser.add_argument('--rotate_angle', default=0, type=int,
...
str_tmp = Structure.from_file(args.filename)  # read in 2D monolayer structure from POSCAR
...
str_cart_coords_layer2[:,2] += (args.interlayer + z_max_layer1 - z_min_layer1)
...
str_lattice[2,2] = math.ceil(args.vacuum + args.interlayer + 2*(z_max_layer1 - z_min_layer1))
...
anchor = args.rotate_anchor[0]*str_bilayer.lattice.matrix[0,:] + args.rotate_anchor[1]*str_bilayer.lattice.matrix[1,:]
str_bilayer.rotate_sites(list(idx_atom_layer2),  # list of rotate-site index
                         args.rotate_angle/180 * np.pi,  # rotation angle in radian
...
str_frac_coords_bilayer[len(str_tmp):len(str_tmp)*2, 0] += args.ashift
str_frac_coords_bilayer[len(str_tmp):len(str_tmp)*2, 1] += args.bshift
...
str_bilayer_shift_sort.to('poscar', 'POSCAR_new')
